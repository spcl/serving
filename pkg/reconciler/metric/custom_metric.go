...
var (
	_ metricreconciler.Interface        = (*customReconciler)(nil)
	_ pkgreconciler.OnDeletionInterface = (*customReconciler)(nil)
)

// ReconcileKind implements Interface.ReconcileKind.
func (r *customReconciler) ReconcileKind(ctx context.Context, metric *autoscalingv1alpha1.Metric) pkgreconciler.Event {
	logger := logging.FromContext(ctx)
	logger.Infof("metric ReconcileKind: Received metric: %v %v %v", metric.Name, metric.Spec.ScrapeTarget, metric.Spec.StableWindow)
...
func (r *customReconciler) ObserveDeletion(ctx context.Context, key types.NamespacedName) error {
